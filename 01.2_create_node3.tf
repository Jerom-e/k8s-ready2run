resource "proxmox_vm_qemu" "node3" {
  name       = var.name_cluster_node3
  agent      = 1
  target_node = var.name_node_node3   # Spécifiez le nom de votre nœud Proxmox ici
...
# Utiliser Cloud-Init pour configurer l'IP et le nom d'hôte unique
  ...
  ipconfig0 = var.interfaces_clusters_ip0_node3  # Remplacez par votre passerelle réseau
